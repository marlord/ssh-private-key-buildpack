...
Add the buildpack to your Heroku app either using the CLI or the Heroku dashboard. The `ssh-private-key-buildpack` needs to run before any buildpack trying to get ssh access. In the following example, it runs before the `heroku/go` buildpack.
    ...
    $ heroku buildpacks:set --index 1 https://github.com/marlord/ssh-private-key-buildpack.git
...
The same example given for the CLI use would have the following `.buildpacks` file.
    ...
    $ cat .buildpacks
    https://github.com/marlord/ssh-private-key-buildpack.git
...
By default, port 22 is used for ssh. For alternative port the parameter SSH_PORT can be used. For example, atlassian bitbucket server uses the port 7999

    $ heroku config:set SSH_PORT="7999"
